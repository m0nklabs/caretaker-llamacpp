"""STT engine lifecycle pins — mirrored from the TTS lifecycle pins.

The caretaker spawns/stops the STT engine from CARETAKER_STT_COMMAND on every
host (same contract as TTS): health-first ensure, idle-stop, VRAM gate with
configurable wait, ensure-lock, deterministic llama-yield, UTF-8 spawn env.
"""
...
from caretaker import stt as stt_mod


@pytest.fixture(autouse=True)
def _reset_state(monkeypatch):
    monkeypatch.setattr(stt_mod, "_last_use_monotonic", None)
    monkeypatch.setattr(stt_mod, "_watcher_task", None)
    monkeypatch.setattr(stt_mod, "_process", None)
    yield


def _patch_env(monkeypatch, **values):
    base = {
        "CARETAKER_STT_ENABLED": "1",
        "CARETAKER_STT_URL": "http://127.0.0.1:11450",
        "CARETAKER_STT_COMMAND": ".venv/bin/python tts_http_wrapper.py",
        "CARETAKER_STT_CWD": "/home/flip/Qwen3-TTS-GGUF",
        "CARETAKER_STT_IDLE_SECONDS": "600",
        "CARETAKER_STT_START_TIMEOUT": "90",
    }
    base.update(values)
    monkeypatch.setattr(stt_mod, "_env", lambda name, default: base.get(name, default))
    monkeypatch.setattr(stt_mod, "_gpu_free_mb", AsyncMock(return_value=8000))
...
def _patch_health(monkeypatch, states):
    ...
    monkeypatch.setattr(stt_mod, "_engine_healthy", _healthy)
...
def _patch_spawn(monkeypatch, process):
    ...
    monkeypatch.setattr(stt_mod.asyncio, "create_subprocess_exec", _spawn)
    return spawned


async def test_disabled_reports_reason(monkeypatch):
    _patch_env(monkeypatch, CARETAKER_STT_ENABLED="0")
    ensure = await stt_mod.ensure_stt()
    release = await stt_mod.release_stt()
...
async def test_ensure_without_command_reports_not_configured(monkeypatch):
    _patch_env(monkeypatch, CARETAKER_STT_COMMAND="")
...
async def test_ensure_start_timeout_reports_failure(monkeypatch):
    _patch_env(monkeypatch, CARETAKER_STT_START_TIMEOUT="0")
    _patch_health(monkeypatch, [False])
...
async def test_release_stops_own_child(monkeypatch):
    ...
    proc = _FakeProcess()
    monkeypatch.setattr(stt_mod, "_process", proc)
    result = await stt_mod.release_stt()
...
async def test_watcher_releases_after_idle(monkeypatch):
    ...
    _patch_env(monkeypatch)
    monkeypatch.setattr(stt_mod, "release_stt", fake_release)
    monkeypatch.setattr(stt_mod, "_engine_healthy", AsyncMock(return_value=True))
    import time as _time

    monkeypatch.setattr(stt_mod, "_last_use_monotonic", _time.monotonic() - 9999)

    async def _sleep(seconds):
        raise asyncio.CancelledError()

    monkeypatch.setattr(stt_mod.asyncio, "sleep", _sleep)
    task = stt_mod.asyncio.create_task(stt_mod._idle_watcher_loop())
    try:
        await task
...
async def test_shlex_splits_windows_command_with_backslashes(monkeypatch):
    ...
    monkeypatch.setattr(stt_mod.os, "name", "nt")
    monkeypatch.setattr(
        stt_mod, "_env",
        lambda name, default: "J:\\Qwen3-TTS-GGUF\\.venv\\Scripts\\python.exe tts_http_wrapper.py"
        if name == "CARETAKER_STT_COMMAND" else default,
    )
    parts = stt_mod._spawn_command()
...
async def test_vram_gate_spawns_when_enough_free(monkeypatch):
    _patch_env(monkeypatch, CARETAKER_STT_MIN_FREE_MB="2500")
    monkeypatch.setattr(stt_mod, "_gpu_free_mb", AsyncMock(return_value=8000))
...
async def test_tts_first_unloads_then_spawns(monkeypatch):
    ...
    engine spawns."""
    _patch_env(monkeypatch, CARETAKER_STT_MIN_FREE_MB="2500", CARETAKER_STT_STOP_LLAMA="1")
    monkeypatch.setattr(stt_mod, "_gpu_free_mb", AsyncMock(return_value=9500))
    _patch_health(monkeypatch, [False, False, True])
    _patch_spawn(monkeypatch, _FakeProcess())
    unloaded = []
...
async def test_vram_gate_fails_honestly_when_llama_stop_disabled(monkeypatch):
    _patch_env(monkeypatch, CARETAKER_STT_MIN_FREE_MB="2500", CARETAKER_STT_STOP_LLAMA="0")
    monkeypatch.setattr(stt_mod, "_gpu_free_mb", AsyncMock(return_value=800))
    _patch_health(monkeypatch, [False])
    result = await stt_mod.ensure_stt()
    assert result["ok"] is False
...
async def test_vram_wait_frees_up_and_spawns(monkeypatch):
    ...
    _patch_env(monkeypatch, CARETAKER_STT_VRAM_WAIT_SECONDS="10", CARETAKER_STT_VRAM_POLL_SECONDS="2")
    free_states = iter([800, 800, 9000])

    async def _free():
        return next(free_states)

    monkeypatch.setattr(stt_mod, "_gpu_free_mb", _free)
...
async def test_vram_wait_times_out_and_gives_up(monkeypatch):
    _patch_env(monkeypatch, CARETAKER_STT_VRAM_WAIT_SECONDS="4", CARETAKER_STT_VRAM_POLL_SECONDS="2")
    monkeypatch.setattr(stt_mod, "_gpu_free_mb", AsyncMock(return_value=800))
    _patch_health(monkeypatch, [False])
    result = await stt_mod.ensure_stt()
    assert result["ok"] is False
...
async def test_concurrent_ensures_spawn_once(monkeypatch):
    ...
    monkeypatch.setattr(stt_mod.asyncio, "create_subprocess_exec", _spawn)
    one, two = await asyncio.gather(stt_mod.ensure_stt(), stt_mod.ensure_stt())
...
async def test_tts_first_host_yields_llama_even_with_vram_plenty(monkeypatch):
    ...
    _patch_env(monkeypatch, CARETAKER_STT_STOP_LLAMA="1")
    monkeypatch.setattr(stt_mod, "_gpu_free_mb", AsyncMock(return_value=9000))
    _patch_health(monkeypatch, [False, False, True])
    _patch_spawn(monkeypatch, _FakeProcess())
    unloaded = []
