...
- **Verificatie:** 7 pins in `tests/test_watchdog_wiring.py` (arm+stop, disable, env-overrides, invalid→default, build-failure fail-open, raisende arming fail-open, hasattr-miss); **174 tests groen** (143 s, lokaal py3.14-venv; CI = py3.12), ruff clean op beide aangeraakte files (I001-importsort meegenomen — bleek pre-existing op HEAD). **PR-Piet ronde 1: 1 terecht bevinding** (fail-open dekte de arming zelf niet — een raisende `start_watchdog()` blokkeerde alsnog de boot) → try-omvanging uitgebreid + pin `test_lifespan_survives_raising_start_watchdog`; daarna opnieuw `/review`.
